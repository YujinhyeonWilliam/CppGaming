...
void LineMesh::Render(HDC hdc, Pos pos, float ratioX, float ratioY) const
{
	for (auto& line : _lines)
	{
		...
		Pos pos1;
		pos1.x = pos.x + static_cast<float>(pt1.x) * ratioX;
		pos1.y = pos.y + static_cast<float>(pt1.y) * ratioY;

		Pos pos2;
		pos2.x = pos.x + static_cast<float>(pt2.x) * ratioX;
		pos2.y = pos.y + static_cast<float>(pt2.y) * ratioY;
